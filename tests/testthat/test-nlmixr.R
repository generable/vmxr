...
test_that("a recorded API 0.2.2 per-basis export (arv-022-dv fixtures) assembles from disk", {
  # Recorded-shape coverage for the deployed 0.2.2 path, the sibling of the 0.3
  # staging-dv replay above: read the on-disk arv-022-dv payloads (not hand-built
  # mocks) and confirm the client handles the faithful 0.2.2 shape.
  fixture <- function(name) {
    jsonlite::fromJSON(test_path("fixtures", "arv-022-dv", paste0(name, ".json")), simplifyVector = FALSE)
  }
  dv_body <- fixture("dv")
