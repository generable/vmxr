...
test_that("vmx_model_data bundles available tables + meta and skips absent ones", {
  ...
  expect_s3_class(md$dosing, "tbl_df")
  expect_null(md$pd)                      # pd not available -> not fetched
  expect_equal(md$meta$n_subjects, 8L)
  expect_equal(i, 3L)                     # only advertised tables fetched
})

test_that("vmx_model_data rejects incomplete table availability metadata", {
  dv <- new_vmx_resource(list(
    data_version_id = "dv_1",
    table_availability = list(subjects = TRUE, pk = TRUE)
  ), "vmx_data_version", "data_version_id")

  expect_error(
    vmx_model_data(dv, client = con),
